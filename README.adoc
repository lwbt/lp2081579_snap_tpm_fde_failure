...
** Enter this command `%s/nvme_1/nvme_2/gc`.
   ...
   Hit enter and confirm every occurence with `y`.
** Close vim with `:wq`
* Check that `snap list` can be executed.
** If it does not execute within a few seconds check `systemctl --failed` and further investgate or restart failed units.
